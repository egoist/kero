...
public class TextLayoutManager: NSObject {
    ...
    /// When true, the next layout pass recomputes ``maxLineWidth`` from all
    /// typeset fragments instead of only growing it. Set after character edits
    /// so shortening (or deleting) a long line shrinks the horizontal scroll
    /// range.
    var maxLineWidthNeedsRecompute = false
