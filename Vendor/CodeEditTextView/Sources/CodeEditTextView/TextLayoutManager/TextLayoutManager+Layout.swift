...
extension TextLayoutManager {
    ...
    public func layoutLines(in rect: NSRect? = nil) -> Set<TextLine.ID> { // swiftlint:disable:this function_body_length
        ...
        if maxLineWidthNeedsRecompute {
            // Full recompute so document width can shrink after edits. Must run
            // after this pass has typeset visible lines, so their new fragment
            // widths are included.
            recomputeMaxLineWidth()
            maxLineWidthNeedsRecompute = false
        } else if maxFoundLineWidth > maxLineWidth {
    ...
    /// Scans every typeset line fragment and updates ``maxLineWidth``. Used
    /// after character edits so the document can shrink when the widest line
    /// is shortened or removed.
    private func recomputeMaxLineWidth() {
        var width: CGFloat = 0
        for linePosition in lineStorage {
            for fragmentPosition in linePosition.data.lineFragments {
                width = max(width, fragmentPosition.data.width)
            }
        }
    ...
    private func layoutLine(
        ...
        // Write through the inout parameter (a local `var maxFoundLineWidth = …`
        // shadow would drop the update and leave document width stuck at the
        // viewport width, so horizontal scrolling never activates).
