...
final class Project: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    /// Text to seed the inline rename field. Keep the displayed project name
    /// but omit live activity decoration so it is not copied into
    /// `customName` and frozen.
    ...
    /// Trims a user-assigned project name and strips a leading Braille
    /// activity frame. Also used while restoring snapshots so names
    ...
    static func normalizedCustomName(_ name: String?) -> String? {
        ...
        // User-entered names are not terminal protocol payloads. Repair the
        // legacy frozen Braille prefix, but never apply Grok's heuristic
        // segment parser to a name the user chose.
        let normalized = braillePrefixedTitleParts(trimmed).name
    ...
    private static func grokCLITitleParts(
        ...
        // compositions unless at least one unambiguous agent-title signal is
        // present. Free-form ellipsis text is not sufficient by itself
        // because it may be a legitimate terminal or session name.
        ...
        guard looksLikeAgentTitle else { return nil }
        ...
        // Free-form activity (tool descriptions) is not a closed set. Only
        // discard an ellipsis / long blurb when another stable label remains;
        // otherwise it may be the user's session name.
        if stable.count >= 2 {
